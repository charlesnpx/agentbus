...
private `<jobId>.items.jsonl` sidecar beside the backend logs. It is not a
`logPaths` field, a durable store field, or a new RPC method. Each item has an
ordinal, append time, closed kind (`message`, `tool`, `fileChange`, `warning`,
or `error`), optional tool name and text, and a truncation flag. Consecutive
agent-text events are coalesced; without a boundary signal in `engine.Event`,
two otherwise adjacent agent messages also coalesce.

Text is capped at 4 KiB per item and the sidecar at 16 MiB. The writer reserves
space for a final `{"appendStopped":true}` control line when the file cap stops
appending, so a later reader can report a gap instead of silently treating the
partial item sequence as complete. Sidecar failures become job diagnostics and
do not alter the operator work's outcome. File-change observations retain only
their kind, name, and timestamp, never their paths or contents.
...
file cap. A contentless progress event advances only the activity timestamp.
