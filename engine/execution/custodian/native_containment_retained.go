...
func (backend *retainedNativeContainmentBackend) containSelfTestRetainedActive(ctx context.Context, group model.GroupRef, params containment.Params, waitForReap func(context.Context) error) PhysicalOutcome {
	if ctx == nil {
		ctx = context.Background()
	}
	if err := backend.requireSelfTestRetainedFixtureRunning(ctx, group); err != nil {
		return unprovablePhysical(group, containment.ReasonProbeUnprovable, model.Unprovable, err)
	}
	result, err := backend.capability.SignalTerm(ctx)
	if err != nil || result != containment.SignalDelivered {
		return unprovablePhysical(group, containment.ReasonSignalUnprovable, model.SignalDirectly, retainedSelfTestSignalError("TERM", result, err))
	}
	if err := sleepContext(ctx, params.GracePeriod); err != nil {
		return unprovablePhysical(group, containment.ReasonContextDone, model.SignalDirectly, err)
	}
	if err := backend.requireSelfTestRetainedFixtureRunning(ctx, group); err != nil {
		return unprovablePhysical(group, containment.ReasonProbeUnprovable, model.Unprovable, err)
	}
	result, err = backend.capability.Kill(ctx)
	if err != nil || result != containment.SignalDelivered {
		return unprovablePhysical(group, containment.ReasonSignalUnprovable, model.SignalDirectly, retainedSelfTestSignalError("KILL", result, err))
	}
	if waitForReap != nil {
		if err := waitForReap(ctx); err != nil {
			return unprovablePhysical(group, containment.ReasonContextDone, model.SignalDirectly, err)
		}
	}
	if err := backend.waitSelfTestRetainedAbsent(ctx, group, params); err != nil {
		reason := containment.ReasonProbeUnprovable
		if errors.Is(err, context.DeadlineExceeded) || errors.Is(err, context.Canceled) {
			reason = containment.ReasonContextDone
		}
		return unprovablePhysical(group, reason, model.SignalDirectly, err)
	}
...
func retainedSelfTestSignalError(signal string, result containment.SignalResult, err error) error {
	message := fmt.Errorf("%w: retained %s result=%v", ErrNativeRuntimeSelfTestUnsafe, signal, result)
	if err == nil {
		return message
	}
	return errors.Join(message, err)
}

func (backend *retainedNativeContainmentBackend) requireSelfTestRetainedFixtureRunning(ctx context.Context, group model.GroupRef) error {
	if backend == nil || backend.capability == nil {
		return fmt.Errorf("%w: retained containment backend is nil", ErrNativeCustodianUnavailable)
	}
	if err := group.Validate(); err != nil {
		return err
	}
	identity := backend.capability.Identity()
	if identity.RetainedID != group.RetainedID || !identity.KernelDomainID.ProvablySame(group.KernelDomain()) {
		return fmt.Errorf("%w: retained self-test capability identity mismatch", ErrNativeRuntimeSelfTestUnsafe)
	}
	membership, err := backend.capability.Membership(ctx)
	if err != nil {
		return fmt.Errorf("%w: read retained self-test membership: %w", ErrNativeRuntimeSelfTestRetry, err)
	}
	if membership != containment.RetainedMembershipPresent {
		return fmt.Errorf("%w: retained self-test fixture was not populated before active containment: membership=%v", ErrNativeRuntimeSelfTestRetry, membership)
	}
	leader, err := observeNativeLeader(group)
	if err != nil {
		return fmt.Errorf("%w: observe self-test leader: %w", ErrNativeRuntimeSelfTestRetry, err)
	}
	if leader.Identity != model.ProcessIdentityMatching || leader.RunState != procgroup.ProcessRunStateRunning {
		return fmt.Errorf("%w: self-test fixture leader not running before active containment: observation=%+v", ErrNativeRuntimeSelfTestRetry, leader)
	}
	absent, err := independentlyAbsent(ctx, group)
	if err != nil {
		return fmt.Errorf("%w: probe self-test process group: %w", ErrNativeRuntimeSelfTestRetry, err)
	}
	if absent {
		return fmt.Errorf("%w: self-test process group absent before active containment", ErrNativeRuntimeSelfTestRetry)
	}
	return nil
}

func (backend *retainedNativeContainmentBackend) waitSelfTestRetainedAbsent(ctx context.Context, group model.GroupRef, params containment.Params) error {
	...
	var lastErr error
	for {
		membership, membershipErr := backend.capability.Membership(ctx)
		if membershipErr != nil {
			...
			if absentErr != nil {
				...
				lastErr = fmt.Errorf("self-test process group is still live after retained kill")
			}
		} else {
			lastErr = fmt.Errorf("self-test retained membership=%v after retained kill", membership)
		}
		if !time.Now().Before(deadline) {
			if lastErr == nil {
				lastErr = fmt.Errorf("self-test retained absence deadline exceeded")
			}
			return lastErr
		}
