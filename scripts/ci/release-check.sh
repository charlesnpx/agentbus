...
cat >/dev/null || true
printf '{"type":"agent_message","text":"release smoke ok\n","thread_id":"release-smoke"}\n'
printf '{"type":"turn.completed","last_agent_message":"release smoke ok\n","thread_id":"release-smoke"}\n'
EOF
...
cat >/dev/null || true
printf '{"type":"assistant","message":{"content":[{"type":"text","text":"release smoke ok\n"}]},"session_id":"release-smoke"}\n'
printf '{"type":"result","result":"release smoke ok\n","session_id":"release-smoke"}\n'
EOF
...
strict_startup_smoke() {
  local smoke_dir bin_dir codex_home home state_root cwd stdout stderr pid code attempts
  ...
  pid=$!
  attempts=0
  while kill -0 "$pid" >/dev/null 2>&1 && ((attempts < 50)); do
    sleep 0.1
    attempts=$((attempts + 1))
  done
  if kill -0 "$pid" >/dev/null 2>&1; then
    ...
    set -e
    printf 'release-check: exact binary strict startup smoke served for %s ms\n' "$((attempts * 100))"
    return 0
  fi
  wait "$pid"
  code=$?
  set -e
  if [[ "$code" -ne 0 ]] && grep -q 'strict admission support unavailable' "$stderr"; then
    printf 'release-check: exact binary strict startup smoke failed closed on unsupported host\n'
    return 0
  fi
  printf 'release-check: exact binary strict startup smoke exit=%d, want serving timeout or strict fail-closed diagnostic\nstdout=%s\nstderr=%s\n' "$code" "$(cat "$stdout")" "$(cat "$stderr")" >&2
  return 1
}
...
run env AGENTBUS_RUN_STRICT_E2E=1 go test -tags abd_strict_e2e ./internal/served -run TestProductionStrict -count=1
